...
if (isset($_GET['id'])) {
    ...
    $product = $db->get(PRODUCT_TABLE, '*', ['id' => $id]);

    if (!$product) {
        flash()->message('danger', 'Không tìm thấy sản phẩm này');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($validation->passes() && $thumbnail->isValid()) {
        ...
        flash()->message('success', 'Cập nhật sản phẩm mới thành công!');
...
?>
    <div class="card">
        <div class="card-header">
            Chỉnh sửa sản phẩm <strong><?= $product['name'] ?></strong>
        </div>
        <div class="card-body">
            ...
            <?php endif; ?>
            <form action="/admin/products/edit.php?id=<?= $product['id'] ?>" method="post" enctype="multipart/form-data">
                <div class="mb-3 row">
                    ...
                    <div class="col-sm-10">
                        <select name="category_id" id="category_id" class="form-select">
                            ...
                            <?php foreach ($categories as $category) : ?>
                                <option value="<?= $category['id'] ?>" <?= selected($category['id'], $_POST['category_id'] ?? $product['category_id']) ?>><?= $category['name'] ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
                <div class="mb-3 row">
                    ...
                    <div class="col-sm-10">
                        <input type="text" class="form-control" id="name" name="name" placeholder="Nhập tên sản phẩm" value="<?= old('name', $product) ?>">
                    </div>
                </div>
                <div class="mb-3 row">
                    <label for="price" class="col-sm-2 col-form-label">Giá bán</label>
                    <div class="col-sm-10">
                        <input type="number" class="form-control" id="price" name="price" placeholder="Nhập giá bán" value="<?= old('price', $product) ?>">
                    </div>
                </div>
                <div class="mb-3 row">
                ...
                <div class="mb-3 row">
                    ...
                    <div class="col-sm-10">
                        <textarea name="description" id="description" class="form-control" rows="3" placeholder="Nhập mô tả sản phẩm"><?= old('description', $product) ?></textarea>
                    </div>
                </div>
                <div class="mb-3 row">
